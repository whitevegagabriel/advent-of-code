...
fn p2(input: &str) -> usize {
    ...
    let mut incorrect_updates = updates.into_iter().filter(|pages| {
        let mut seen = HashSet::new();
        for page in pages.iter() {
            seen.insert(*page);
    ...
    for incorrect_update in incorrect_updates.iter_mut() {
        ...
        incorrect_update.sort_by_key(|n| {
            rankings.get(n).unwrap_or(&0)
        })
    }
    
    incorrect_updates.iter().map(|pages| pages[pages.len()/2]).sum()
}

fn generate_rankings(rules: &HashMap<usize, Vec<usize>>, values: &[usize]) -> HashMap<usize, usize> {
    let mut new_rules = rules.clone();
    let values: HashSet<_> = values.iter().collect();
    
    new_rules.retain(|k, _| {
        values.contains(k)
    });
...
fn recursive_increment(left: &usize, rules: &HashMap<usize, Vec<usize>>, rankings: &mut HashMap<usize, usize>) {
