...
fn p1(input: &str) -> usize {
    ...
    grid.iter().filter_map(|(k, v)| {
        if v != &0 {
            return None;
        }
        
        Some(reachable_peaks_with_repetition(*k, &grid).iter().unique().count())
    }).sum()
}
...
fn reachable_peaks_with_repetition(curr: Point2<isize>, grid: &HashMap<Point2<isize>, usize>) -> Vec<Point2<isize>> {
    let curr_height = grid[&curr];
    get_cross_neighbors(curr).iter().filter_map(|neighbor| {
